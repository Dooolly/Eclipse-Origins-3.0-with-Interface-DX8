...
Dim Buffer As clsBuffer, X As Long
    ...
    ' check if spell
    If Hotbar(Slot, ActualHotbar).sType = 2 Then ' spell
        For X = 1 To MAX_PLAYER_SPELLS
            ' is the spell matching the hotbar?
            If PlayerSpells(X).Num = Hotbar(Slot, ActualHotbar).Slot Then
                ' found it, cast it
                CastSpell X
                Exit Sub
            End If
...
Sub Send_PlusSpell(ByVal spellslot As Byte)
    Dim Buffer As clsBuffer
    ...
    Set Buffer = New clsBuffer
    Buffer.WriteLong CSendPlusSkill
