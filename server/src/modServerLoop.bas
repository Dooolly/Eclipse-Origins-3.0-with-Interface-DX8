            ...
            For i = 1 To Player_HighIndex
                If IsPlaying(i) Then
                    ...
                    If TempPlayer(i).spellBuffer.Spell > 0 Then
                        If GetTickCount > TempPlayer(i).spellBuffer.Timer + (Spell(Player(i).Spell(TempPlayer(i).spellBuffer.Spell).Num).CastTime * 1000) Then
...
Public Sub UpdateMapBlock(ByVal mapNum As Byte, ByVal X As Integer, ByVal Y As Integer, ByVal Blocked As Boolean)
    If Blocked Then
        MapBlocks(mapNum).Blocks(X, Y) = 9
    Else
