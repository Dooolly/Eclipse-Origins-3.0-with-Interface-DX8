...
Sub ClearQuest(ByVal Index As Long)
    ...
    Quest(Index).StartMessage = vbNullString
    Quest(Index).MidMessage = vbNullString
    Quest(Index).FinishMessage = vbNullString
End Sub
...
Public Sub SendPlayerQuests(ByVal Index As Integer)
    ...
    Buffer.WriteLong SPlayerQuest
        ...
        For i = 1 To MAX_QUESTS
            Buffer.WriteByte Player(Index).PlayerQuest(i).Status
            Buffer.WriteByte Player(Index).PlayerQuest(i).ActualTask
            Buffer.WriteByte Player(Index).PlayerQuest(i).CurrentCount
        Next
...
Public Sub EndQuest(ByVal Index As Long, ByVal QuestNum As Long)
    ...
    'show ending message
    QuestMessage Index, QuestNum, Trim$(Quest(QuestNum).FinishMessage), 0
