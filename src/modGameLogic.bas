...
Dim fogtmr As Long, TmrChat As Long
        ...
        If tmr25 < tick Then
            ...
            If NumSpellIcons > 0 Then
                For i = 1 To MAX_PLAYER_SPELLS
                    If PlayerSpells(i).Num > 0 Then
                        If PlayerSpells(i).Cooldown > 0 Then
                            If PlayerSpells(i).Cooldown + (Spell(PlayerSpells(i).Num).CDTime * 1000) < tick Then
                                PlayerSpells(i).Cooldown = 0
                            End If
                        End If
                    End If
                Next
            ...
            If SpellBuffer > 0 Then
                If SpellBufferTimer + (Spell(PlayerSpells(SpellBuffer).Num).CastTime * 1000) < tick Then
                    SpellBuffer = 0
                    SpellBufferTimer = 0
                End If
            End If
...
Dim buffer As clsBuffer
    ...
    If PlayerSpells(spellslot).Cooldown > 0 Then
...
Dim buffer As clsBuffer
    ...
    If PlayerSpells(spellslot).Num = 0 Then Exit Sub
    ...
    If PlayerSpells(spellslot).Num > 0 Then
        If GetTickCount > Player(MyIndex).AttackTimer + 1000 Then
            ...
            Else
                Call AddText("Não dá para usar jutsus em movimento!", BrightRed)
            End If
        End If
    End If
...
Private Sub ScrollChatBox(ByVal Direction As Byte)
